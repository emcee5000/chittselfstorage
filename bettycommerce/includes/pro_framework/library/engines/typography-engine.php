...
function upfw_google_fonts(){
    ...
    $upfw_google_fonts = array(
		...
		'Helvetica Neue', Helvetica, 'Maven Pro', 'Myriad Pro', 'Myriad' => array('weights' => array('Normal' => '400',
												'Medium' => '500',
												'Bold' => '700',
												'Ultra-Bold' => '900') ),
...
function upfw_multiple_typography($options){
    ...
    $multiple = array(
        array(
        'name' => __('Selector Groups', PA_TEXTDOMAIN),
        'desc' => __('Add a new selector group (comma delimited) to generate a font style option below. You must save the options first.', PA_TEXTDOMAIN),
        'id' => 'upfw_user_selectors',
        'type' => 'text_list',
        'default_text' => __('Add New Selector Group', PA_TEXTDOMAIN))
    );
    ...
    if( isset( $up_options->upfw_user_selectors ) ) :		
        foreach( $up_options->upfw_user_selectors as $name ):
            $multiple[] = array(
                'name' => $name,
                'desc' => __( 'Custom Selectors', PA_TEXTDOMAIN ),
                'type' => 'typography',
