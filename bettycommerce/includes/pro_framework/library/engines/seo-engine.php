...
add_action('wp_head', 'up_seo_init');


/* Check for SEO Plugins */
function up_seo_third_parties(){
	if ( ! defined( 'UP_SEO_THIRD_PARTY' ) ) define( 'UP_SEO_THIRD_PARTY', false );
    if( class_exists( 'All_in_One_SEO_Pack' ) || class_exists( 'Headspace_Plugin' ) || class_exists( 'WPSEO_Frontend' ) ) return true;
...
function up_seo_default_options() {
    ...
    if ( up_seo_third_parties() )echo "<kbd>".__('We detected that a third-party SEO plugin has been activated. Pro Affiliate SEO has been automatically disabled to avoid any conflict.', PA_TEXTDOMAIN)."</kbd>";
    
    $options = array (
        
            array(	"name" => __('Disable the Pro Affiliate SEO?','upfw'),
                            "desc" => __("Useful when using third-party plugins.",'upfw'),
                            "id" => "seo_disable",
                            "value" => "",
                            "type" => "select",
                            "default_text" => __('No', PA_TEXTDOMAIN),
                            "options" => array(
                                __('Yes', PA_TEXTDOMAIN)=> '1'
                            )
            ),
            
            array(	"name" => __('Hide Custom Fields?','upfw'),
                            "desc" => __("If enabled, Pro Affiliate SEO options will not be displayed on posts/pages.",'upfw'),
            ...
            array(	   "name" => __('Homepage Title Layout','upfw'),
                            ...
                            "type" => "select",
                            "default_text" => __('Default', PA_TEXTDOMAIN),
                            "options" => array(
                                __('Blog Title | Description', PA_TEXTDOMAIN)=> '%BLOG% %DESC%',
                                __('Blog Title', PA_TEXTDOMAIN)=> '%BLOG%',
                                __('Description', PA_TEXTDOMAIN)=> '%DESC%',
                                __('Homepage Title', PA_TEXTDOMAIN)=> '%TITLE%',
                                __('Homepage Title | Description', PA_TEXTDOMAIN)=> '%TITLE% %DESC%'
                            )
            ),
            ...
            array(	"name" => __('Page Title Layout','upfw'),
                            ...
                            "default_text" => __('Default', PA_TEXTDOMAIN),
                            "options" => array(
                                __('Title | Blog Title | Description', PA_TEXTDOMAIN)=> '%TITLE% %BLOG% %DESC%',
                                __('Blog Title | Title | Description', PA_TEXTDOMAIN)=> '%BLOG% %TITLE% %DESC%',
                                __('Title | Blog Title', PA_TEXTDOMAIN)=> '%TITLE% %BLOG%',
                                __('Title | Description', PA_TEXTDOMAIN)=> '%TITLE% %DESC%',
                                __('Title', PA_TEXTDOMAIN)=> '%TITLE%'
                            )
            ),
            ...
            array(	"name" => __('Archive Title Layout','upfw'),
                            ...
                            "default_text" => __('Default', PA_TEXTDOMAIN),
                            "options" => array(
                                __('Archive Name | Page Number | Blog Title | Description', PA_TEXTDOMAIN)=> '%ARCHIVE% %PAGED% %BLOG% %DESC%',
                                __('Page Number | Archive Name | Blog Title | Description', PA_TEXTDOMAIN)=> '%PAGED% %ARCHIVE% %BLOG% %DESC%',
                                __('Page Number | Archive Name | Blog Title', PA_TEXTDOMAIN)=> '%PAGED% %ARCHIVE% %BLOG%',
                                __('Archive Name | Page Number | Blog Title', PA_TEXTDOMAIN)=> '%ARCHIVE% %PAGED% %BLOG%',
                                __('Archive Name | Blog Title | Description', PA_TEXTDOMAIN)=> '%ARCHIVE% %BLOG% %DESC%',
                                __('Blog Title | Archive Name | Description', PA_TEXTDOMAIN)=> '%BLOG% %ARCHIVE% %DESC%',
                                __('Archive Name | Blog Title', PA_TEXTDOMAIN)=> '%ARCHIVE% %BLOG%',
                                __('Archive Name | Description', PA_TEXTDOMAIN)=> '%ARCHIVE% %DESC%',
                                __('Archive Name', PA_TEXTDOMAIN)=> '%ARCHIVE%'
                            )
            ),
            ...
            array(	"name" => __('Search Results Title Layout','upfw'),
                            ...
                            "default_text" => __('Default', PA_TEXTDOMAIN),
                            "options" => array(
                                __('Search Results | Page Number | Blog Title | Description', PA_TEXTDOMAIN)=> '%SEARCH% %PAGED% %BLOG% %DESC%',
                                __('Page Number | Search Results | Blog Title | Description', PA_TEXTDOMAIN)=> '%PAGED% %SEARCH% %BLOG% %DESC%',
                                __('Page Number | Search Results | Blog Title', PA_TEXTDOMAIN)=> '%PAGED% %SEARCH% %BLOG%',
                                __('Search Results | Page Number | Blog Title', PA_TEXTDOMAIN)=> '%SEARCH% %PAGED% %BLOG%',
                                __('Search Results | Blog Title | Description', PA_TEXTDOMAIN)=> '%SEARCH% %BLOG% %DESC%',
                                __('Blog Title | Search Results | Description', PA_TEXTDOMAIN)=> '%BLOG% %SEARCH% %DESC%',
                                __('Search Results | Blog Title', PA_TEXTDOMAIN)=> '%SEARCH% %BLOG%',
                                __('Search Results | Description', PA_TEXTDOMAIN)=> '%SEARCH% %DESC%',
                                __('Search Results', PA_TEXTDOMAIN)=> '%SEARCH%'
                            )
            ),
            ...
            array(	"name" => __('Indexing','upfw'),
                            ...
                            "type" => "checkbox",
                            "options" => array(
                                __('Category', PA_TEXTDOMAIN)=> 'category',
                                __('Search', PA_TEXTDOMAIN)=> 'search',
                                __('Author', PA_TEXTDOMAIN)=> 'author',
                                __('Date', PA_TEXTDOMAIN)=> 'date',
                                __('Tag', PA_TEXTDOMAIN)=> 'tag',
                                __('Pages', PA_TEXTDOMAIN)=> 'page',
                                __('Posts', PA_TEXTDOMAIN)=> 'post',
                                __('Home', PA_TEXTDOMAIN)=> 'home'
                            )
            )
...
function up_title(){
	...
	$home_layout = $up_options->seo_home_title_layout ? $up_options->seo_home_title_layout : "%TITLE% %BLOG%";
    $page_layout = $up_options->seo_page_title_layout ? $up_options->seo_page_title_layout : "%TITLE% %BLOG%";
	$single_layout = $up_options->seo_single_title_layout ? $up_options->seo_single_title_layout : "%TITLE% %BLOG%";
	$archive_layout = $up_options->seo_archive_title_layout ? $up_options->seo_archive_title_layout : "%ARCHIVE% %TITLE%";
    $search_layout = $up_options->seo_search_title_layout ? $up_options->seo_search_title_layout : "%SEARCH% %TITLE%";
...
function up_seo_title_layout( $layout = "%ARCHIVE% %TITLE% %BLOG%" ){
    global $up_options, $post, $wp_query;
	
    $sep = $up_options->seo_separator ? ' '.$up_options->seo_separator.' ' : ' | ';
    $paged = get_query_var('paged') ? __('Page ', PA_TEXTDOMAIN).get_query_var('paged') : '';
    $search = get_query_var('s') ? __('Search Results For ', PA_TEXTDOMAIN).get_query_var('s') : '';
	...
	$site_title = ( empty( $up_options->seo_homepage_title ) ) ? get_bloginfo('name') : $up_options->seo_homepage_title;

    /* Insert Blog Title */
    $layout = preg_replace('/%BLOG%/', $site_title, $layout);
...
function up_seo_meta_box() {
    ...
    if(!$up_options->seo_hide_custom_fields):
        if( function_exists( 'add_meta_box' ) ) {
            add_meta_box( 'new-post-meta-boxes', __('Pro Affiliate SEO Options'), 'up_seo_post_meta_display', 'post', 'normal', 'core' );
            add_meta_box( 'new-post-meta-boxes', __('Pro Affiliate SEO Options'), 'up_seo_post_meta_display', 'page', 'normal', 'core' );
			add_meta_box( 'new-post-meta-boxes', __('Pro Affiliate SEO Options'), 'up_seo_post_meta_display', 'sales_letter', 'normal', 'core' );
			add_meta_box( 'new-post-meta-boxes', __('Pro Affiliate SEO Options'), 'up_seo_post_meta_display', 'offer', 'normal', 'core' );
			add_meta_box( 'new-post-meta-boxes', __('Pro Affiliate SEO Options'), 'up_seo_post_meta_display', 'squeeze', 'normal', 'core' );
        }
    endif;
}
    ...
    <div class="form-wrap">
      ...
      <div class="form-field">
        ...
        <select name="meta[996][value]" id="meta[996][value]">
            <option <?php if($follow == 'default')echo "selected='selected'";?> value="default"><?php _e('Default', PA_TEXTDOMAIN);?></option>
            <option <?php if($follow == 'no')echo "selected='selected'";?> value="no"><?php _e('No', PA_TEXTDOMAIN);?></option>
            <option <?php if($follow == 'yes')echo "selected='selected'";?> value="yes"><?php _e('Yes', PA_TEXTDOMAIN);?></option>
        </select>
